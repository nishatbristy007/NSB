...
def TK4_Version2(m,freq,Hamming):   
    #Hamming = 1-(2*jaccard/(1+jaccard))**(1/31)  
    if np.all(m==0):
        return 0
    sums = np.sum(m,axis = 1)
    w = 2*freq[0]
    r1 = (m[0][1] + m[1][0] + m[2][3] + m[3][2])/4 	#r1=F[[1, 2]]+F[[2,1]]+F[[3,4]]+F[[4,3]]
    p1 = (m[0][2] + m[2][0] + m[1][3] + m[3][1])/4 	#p1 = F[[3, 1]]+F[[1,3]]+F[[2,4]]+F[[4,2]]
    q1 = (m[0][3] + m[3][0])/4 	#q1 = F[[1, 4]]+F[[4,1]]
    q2 = (m[1][2] + m[2][1])/4	#q2 = F[[3, 2]]+F[[2,3]]
    s13 = w - (m[0][1] + m[0][2] + m[0][3] + m[3][0] + m[3][1] + m[3][2])/4   				#s13 = F[[1, 1]] + F[[4, 4]]#
    s24 = 1 - w - (m[1][0] + m[1][2] + m[1][3] + m[2][0] + m[2][1] + m[2][3])/4					#s24=F[[2, 2]] + F[[3, 3]] #
    
    ln1 = -1/4*np.log(((s13-q1)*(s24-q2)-((p1-r1)/2)**2)/(w*(1-w)))
    ln2 = -1/4*np.log((1-((p1+r1)/(2*w*(1-w))))**(8*w*(1-w)-1))
    sys.stderr.write("w={0} r1={1} p1={2} q1={3} q2={4} s13={5} s24={6} ln1={7} ln2={8} freq_0={9}".format(w,r1,p1,q1,q2,s13,s24,ln1,ln2,freq[0]))
    return ln1+ln2
