...
def  two_way_genome_builder(path, dir_2way_path):	
    ...
    for file in files:
        ...
        newseq2 = oldseq+'\n'+'\n'.join(newlist)						# Concatenating actual string with it's RC.
...
class Mash_Skmer:
    ...
    def replacer(self,x,y): 
        if os.path.exists(self.replaced_dir):
            shutil.rmtree(self.replaced_dir)
        os.mkdir(self.replaced_dir)

        samples = os.listdir(self.two_way_dir)

        for sample in samples:
            sample_1 = open(self.two_way_dir+'/'+sample, 'r')
            Lines = sample_1.readlines()
            sample_2 = open(self.replaced_dir+'/'+sample,'w') 
            newseq="" 
            for line in Lines:
...
class Jellyfish:
    ...
    def createKmerStats(self):
        ...
        taxafiles = [f for f in listdir(dir_name) if isfile(join(dir_name, f))]
        call(["mkdir","-p",kmer_dir],stderr=open(os.devnull, 'w'))
        for file in taxafiles:
            pathname=dir_name+"/"+file
            outputdir=kmer_dir+"/"+file
            call(["jellyfish", "count", "-m", str(self.k), "-s", str(self.s),pathname, "-o", "tmp.jf"], stderr=open(os.devnull, 'w'))
            call(["jellyfish", "dump", "-c", "tmp.jf","-o",outputdir], stderr=open(os.devnull, 'w'))
            print(file+"jellyfish dump done")
    ...
    def jellyfish_runner(self):
        ...
        if self.method == 2:
            jac.save_encoding(self.kmer_dir,self.k)

        for i in range(self.n_taxa-1):
            for j in range(i,self.n_taxa):
...
def reference(args):
    ...
    print('Building 2way genome of the sequences..')
    ...
    if args.m == 1 or args.m == 2:  # Run Jellyfish
        print("Running for the method with jellyfish")
